print.UGPCM <- function(x, ...){
  
  cat("Output UGPCM estimation:","\n")
  ...
  slopes.mat <- cbind(c(t(x$slopes)), c(t(x$se.slopes)))
  slopes.mat <- cbind(slopes.mat, slopes.mat[,1]/slopes.mat[,2])
  slopes.mat <- cbind(slopes.mat, 2*pnorm(-abs(slopes.mat[, 3])))
  colnames(slopes.mat) <- c("Estimate", "Std.Err", "Z value", "Pr(>z)")
  rownames(slopes.mat) <- names(x$slopes)
  ...
  cat("Estimates of item slope parameters","\n")
  printCoefmat(slopes.mat, ...)
